...
/********************************************************************
* FUNCTION vyatta_agt_record_error
*
* Generate an rpc_err_rec_t and save it in the msg
*
* INPUTS:
...
*********************************************************************/
void
    vyatta_agt_record_error (ses_cb_t *scb,
                      xml_msg_hdr_t *msghdr,
                      ncx_layer_t layer,
                      status_t  res,
                      const xml_node_t *xmlnode,
		              ...
		              char *errormsg)
{
    ncx_errinfo_t *errinfo = NULL;
    if(errormsg != NULL) {
        errinfo = (ncx_errinfo_t *)malloc(sizeof(ncx_errinfo_t));
	    errinfo->error_message = (xmlChar *)errormsg;
	    errinfo->error_app_tag = NULL;
	    errinfo->ref = NULL;
	    errinfo->descr = NULL;
    }
    agt_record_error_errinfo(scb, msghdr, layer, res, xmlnode, parmtyp, 
                             error_info, nodetyp, error_path, errinfo);
    if(errinfo != NULL)
    {
	    if(errinfo->error_message != NULL)
	        free(errinfo->error_message);
            free(errinfo);
    }

} /* vyatta_agt_record_error */
