                  ...
                  char *argv[])
{
    ...
    int my_argc = 1;
    char *my_argv[1];
    my_argv[0] = (char *)malloc(strlen(argv[0]) + 1);
    memset(my_argv[0], '\0', strlen(argv[0]) + 1);
    strcpy(my_argv[0], argv[0]);

    res = ncx_init(NCX_SAVESTR, log_level, TRUE, NULL, my_argc, my_argv);
    if (res != NO_ERR) {
        return res;
    }
    ...
    res = process_cli_input(server_cb, my_argc, my_argv);
    if (res != NO_ERR) {
        return res;
    }
                           ...
                           mgr_rpc_rpy_t *rpy)
{
    ...
    //if(xml_strcmp(req->data->name,(xmlChar *)"get") != 0) {
    //    log_warn("%d", xml_strcmp(req->data->name,(xmlChar *)"get"));
    //    SET_ERROR(ERR_INTERNAL_PTR);
    //    return;
    //}
#endif
    //if(!xml_strcmp(req->data->name,(xmlChar *)"get"))
    //    return;
    ...
    if (rpy && rpy->reply) {
        if (val_find_child(rpy->reply, 
            ...
            if (server_cb->command_mode == CMD_MODE_NORMAL || LOGDEBUG) {
                ...
                ///////////////            
                val_value_t *chval, *subchval;
                char *errorpath = NULL;
                char *errormsg = NULL;
                for (chval = (val_value_t *)dlq_firstEntry(&(rpy->reply->v.childQ));
                            chval != NULL;
                            chval = (val_value_t *)dlq_nextEntry(chval))
                {
                    for (subchval = (val_value_t *)dlq_firstEntry(&(chval->v.childQ));
                            subchval != NULL;
                            subchval = (val_value_t *)dlq_nextEntry(subchval))
                    {
                        if(!strcmp("error-path", (const char *)subchval->name)) {
                            errorpath = VAL_STR(subchval);
                            continue;
                        }
                            
                        if(!strcmp("error-message", (const char *)subchval->name)) {
                            errormsg = VAL_STR(subchval);
                            continue;
                        }
                    }
                }
                
                if(strcmp(errormsg, "data missing"))
                {
                    currentCommand = (currentCommand - 2);
                    free(commands[currentCommand]);
                    char *executeCommand = (char *) malloc(strlen("delete ") + strlen(errorpath) + 1);
                    sprintf(executeCommand, "%s%s", "delete ", errorpath);
                    commands[currentCommand] = (xmlChar *)executeCommand;
                }
            ...
            if (server_cb->echo_replies) {
                ...
                //         rpy->msg_id, 
                //         usesid);
                //log_info("\nReply is <<%s>>",(char *)rpy->reply->name);
                val_dump_value_max(rpy->reply, 
                                   0,
                                   server_cb->defindent,
                                   DUMP_VAL_LOG,
...
/* parse_arguments_and_fill_commands */

status_t parse_arguments_and_fill_commands(int argc, char *argv[])
{
    status_t res = NO_ERR;
    if (!(argc < 6)) {    
        char *connectCommand = (char *) malloc(strlen("connect") + strlen(argv[1]) + strlen(argv[2]) + strlen(argv[3]) + strlen(argv[4]) + 6);
        sprintf(connectCommand, "%s %s %s %s %s", "connect", argv[1], argv[2], argv[3], argv[4]);
        commands[0] = (xmlChar *)connectCommand;

        char *executeCommand = (char *) malloc(strlen(argv[5]) + 1);
        sprintf(executeCommand, "%s", argv[5]);
        commands[1] = (xmlChar *)executeCommand;

        char *commitCommand = (char *) malloc(strlen("commit") + 1);
        sprintf(commitCommand, "%s", "commit");
        commands[2] = (xmlChar *)commitCommand;

        char *quitCommand = (char *) malloc(strlen("quit") + 1);
        sprintf(quitCommand, "%s", "quit");
        commands[3] = (xmlChar *)quitCommand;
    }
    else
        res = SET_ERROR(ERR_INTERNAL_VAL);
...
int main (int argc, char *argv[])
{
    ...
    mtrace();
#endif
    res = parse_arguments_and_fill_commands(argc, argv);
    if (res != NO_ERR) {
