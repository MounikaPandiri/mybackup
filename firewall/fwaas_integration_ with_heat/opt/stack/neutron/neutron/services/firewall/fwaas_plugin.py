...
class FirewallPlugin(firewall_db.Firewall_db_mixin):
    ...
    def create_firewall(self, context, firewall):
        ...
        #module = importutils.import_module(nova)
        nc = nova.Nova_API()
        #servs = nc.get_servers(context)
        #import pdb;pdb.set_trace()
        firewall_policy_id=firewall['firewall']['firewall_policy_id']
        if firewall_policy_id:
            fw_policy = self.get_firewall_policy(context, firewall_policy_id)
            fw_with_rules = [self.get_firewall_rule(
                context, rule_id) for rule_id in fw_policy['firewall_rules']]
            firewall['firewall_rule_list'] = fw_with_rules
        else:
            firewall['firewall_rule_list'] = []    
        instance_name=firewall['firewall']['name']
        fw_source_address=fw_with_rules[0]['source_ip_address']
        fw_destination_address=fw_with_rules[0]['destination_ip_address']
        servs = nc.get_servers(context)
        server_net_id,mgnt_net_id= nc.launch_fw_vm(context,fw_source_address,fw_destination_address,instance_name)
        firewall['firewall']['server_id'] =server_net_id[0].id

        ## Code added by Anirudh on 7-March-2013, 11:59 A.M.
        #self.launch_fw_vm(context,firewall)
        ##
        if not firewall['firewall']['server_id']:
            firewall['firewall']['server_id'] = uuidutils.generate_uuid()
        ...
        fw_with_rules['gateway_id']=server_net_id[1]
        fw_with_rules['mgnt_net_id'] = mgnt_net_id
