...
class VNFManager(manager.Manager):
    ...
    def update_vdu_configuration(self, context, vdu_name, configuration, cfg_engine, software):
        try:
            vdu_name = vdu_name
            if cfg_engine=='puppet':
                arguments = self._generate_config_string(cfg_engine, software)
            else:
    ...
    def _generate_config_string(self, cfg_engine, software):
                    ...
                    for new_vdu in range(0, len(conf[vnfd])):
                        ...
                        for exist_vdu in range(0, len(self.ns_config[vnfd])):
                            if new_vdu_name == self.ns_config[vnfd][exist_vdu]['name']:
                                ...
                                self._configure_vdu(new_vdu_name, 'configure', self.drv_conf[new_vdu_name]['_lc_events']['init'], scale=True)
                                break 
