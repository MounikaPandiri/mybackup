...
class homer(object):
    ...
    def parse(self,cfg):
        for i in range(0, len(cfg['Ims'])):
            ...
            if(cfg['Ims'][i]['name']=="vHomer"):
                ...
                self.localip=cfg['Ims'][i]['vm_details']['network_interfaces']['pkt-in']['ips']['ims-vhomer']
                self.mgmtip=cfg['Ims'][i]['vm_details']['network_interfaces']['management-interface']['ips']['ims-vhomer']
    ...
    def configure_service(self, *args, **kwargs):
         ...
         command_list.append("replace /ims/Service/name --value=homer")
         print(command_list)
