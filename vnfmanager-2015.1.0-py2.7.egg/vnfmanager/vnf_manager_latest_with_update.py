...
class VNFManager(manager.Manager):
    ...
    def update_vdu_configuration(self, context, vdu_name, configuration):
        try:
            ...
            configuration_event = configuration.keys()[0]
    ...
    def _configure_vdu(self, vdu_name, configuration_event='init', scale=False, update=False):
        status = ""
    ...
    def _invoke_driver_thread(self, vdu, instance, vdu_name, mgmt_ip, configuration_event, scale=False):
        global launchd_threads_info
        LOG.debug(_("Configuration of the remote VNF %s being intiated"), instance)
        _thr_id = str(uuid.generate_uuid()).split('-')[1]   
        try:
            if scale:
                driver_thread = ImplThread(self._configure_service, self._condition, vdu, instance, mgmt_ip, configuration_event, _id = _thr_id)
            else:
                driver_thread = ImplThread(self._configure_service, self._condition, vdu, instance, mgmt_ip, configuration_event, _id = _thr_id)
            self._condition.acquire()
