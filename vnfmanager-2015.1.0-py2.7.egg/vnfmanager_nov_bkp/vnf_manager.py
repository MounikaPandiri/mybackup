...
class VNFManager(manager.Manager):
        ...
        while True:
            ...
            for each_thread in iter(thread_info.keys()):
                if 'result' in thread_info[each_thread].keys():
                    ...
                    status = 'ERROR' if 'ERROR' in str(thread_info[each_thread]['result']) else self.status.rstrip('PENDING')+'COMPLETE'
                    self.status = status
        ...
        for vnfd in vnfds:
            for vdu in range(0,len(vnfm_conf[vnfd])):
                ...
                if vdu_name not in self.drv_conf.keys():
                    ...
                    if 'mgmt-driver' in vnfm_conf[vnfd][vdu] and vnfm_conf[vnfd][vdu]['mgmt-driver'] is not '':
                        vnfd_details[vdu_name]['_driver'] = vnfm_conf[vnfd][vdu]['mgmt-driver']
                    else:
                        vnfd_details[vdu_name]['_driver'] = None
    ...
    def wait_for_configuraton_completion(self):
    ...
    def update_vdu_configuration(self, context, vdu_name, configuration):
        try:
            ...
            #self.wait_for_configuraton_completion()
        except Exception:
            self.status = 'UPDATION FAILED'
        finally:
            return self.status
            #ENDS HERE
    ...
    def _generate_config_string(self, cfg_engine):
        ...
        #config_string = '<'+cfg_engine+'>'+'<'+'command'+'>'+"puppet agent -t"+'<'+'/'+'command'+'>'+'<'+'/'+cfg_engine+'>'
    ...
    def configure_vdus(self, context, conf):
        curr_drv_conf = dict()
        vnfds = list(set(conf['service'].keys()) - set(['nsd_id' , 'id', 'fg', 'monitor']))
        for vnfd in vnfds:
                            ...
                            if new_vdu_name == self.ns_config[vnfd][exist_vdu]['name']:
                                ...
                                if new_vdu_name in self.drv_conf.keys():
                                    if vnfd == self.drv_conf[new_vdu_name]['_vnfd']:
                                        ...
                                        self.drv_conf[new_vdu_name]['_lc_events']['scaled_init'] = conf[vnfd][new_vdu]['lifecycle_events']['init']
                                self._configure_vdu(new_vdu_name, 'configure', self.drv_conf[new_vdu_name]['_lc_events']['scaled_init'], scale=True)
                                break 
    ...
    def send_ack(self, context, vnfd, vdu, instance, status, nsd_id, gen_conf):
        return self.call(context,
                         self.make_msg('send_ack', vnfd=vnfd, vdu=vdu,
                                        instance=instance, status=status, nsd_id=nsd_id, gen_conf=gen_conf))
