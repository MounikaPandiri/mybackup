...
class NetConf(abs_driver.AbstractDriver):
    def __init__(self, *args, **kwargs):
        self.masklen = "24"
        self.classpath = "/home/tcs/Netconf.jar:/home/tcs/NetconfClient.jar"
        self.addl_env = {}
    ...
    def _configure_firewall(self, mgmt_ip, args_dict):
        ...
        #else:
        if 'dst_zone' in args_dict.keys() and 'src_zone' in args_dict.keys():
            dst_zone_name = args_dict['dst_zone']
            src_zone_name = args_dict['src_zone']
        elif 'dst_zone' in args_dict.keys() and src_interface:
            dst_zone_name = args_dict['dst_zone']
            src_zone_name = self._get_vsrx_interface_zone(src_interface, self.zone_list)
        elif 'src_zone' in args_dict.keys() and dst_interface:
            dst_zone_name = self._get_vsrx_interface_zone(dst_interface, self.zone_list)
            src_zone_name = args_dict['src_zone']
        else:
            dst_zone_name = self._get_vsrx_interface_zone(dst_interface, self.zone_list)
        ...
        if src_zone_name is not None:
            if 'packet' in args_dict.keys():
                policy_commands_list.append("delete security zones security-zone " + src_zone_name +" screen")
                policy_commands_list.append("set security zones security-zone " + src_zone_name +" tcp-rst")
        ...
        if self.policy_dict:
            for policy in self.policy_dict:
                if self.policy_dict[policy]['src_addr'] == src_ip and self.policy_dict[policy]['dst_addr'] == dst_ip:
                   ...
                   self._configure_firewall(mgmt_ip, args_dict)
        else:
           self._configure_firewall(mgmt_ip, args_dict)
