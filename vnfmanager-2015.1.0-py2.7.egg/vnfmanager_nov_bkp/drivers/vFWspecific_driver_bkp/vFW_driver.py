...
####ends here ############ 
    ...
    def push_configuration(self, instance, mgmt_ip):
        ...
        source_ip = self.ns_config['firewall'][0]['vm_details']\
                    ['network_interfaces']['pkt-in']['ips']['firewall-vfw']
        dest_ip = self.ns_config['firewall'][0]['vm_details']\
                  ['network_interfaces']['pkt-out']['ips']['firewall-vfw']

        self.args_conf['nat']['src_ip'] = source_ip
        self.args_conf['nat']['dst_ip'] = dest_ip

        self.args_conf['fw']['src_ip'] = source_ip
        self.args_conf['fw']['dst_ip'] = dest_ip
    ...
    def _configure_firewall(self, args_dict):
        ...
        src_addr,dst_addr = self._get_addr_from_ip(src_ip, dst_ip)
        ...
        if dst_zone_exists and src_zone_exists:
            ...
            policy_commands_list = self._frame_command_list(src_zone_name, dst_zone_name, src_addr, dst_addr, firewall_action, 
                                     application, policy_commands_list)
...
### ADDED By Mounika for update ###
    ...
    def _frame_command_list(self, src_zone_name, dst_zone_name, 
                            src_addr, dst_addr, firewall_action, 
                            application, policy_command_list):
        ...
        policy_commands_list.append("set security policies from-zone "+ src_zone_name + \
                                    ...
                                    " match source-address " + src_addr_str + \
                                    " destination-address " + dst_addr_str + \
                                    " application " + application)

        policy_commands_list.append("set security policies from-zone " + dst_zone_name + \
                                    " to-zone " + src_zone_name + " policy " + policy_name + \
                                    " match source-address " + dst_addr_str + \
                                    " destination-address " + src_addr_str + \
                                    " application " + application)
        ...
        for policy in self.policy_dict:
            if self.policy_dict[policy]['src_addr'] == src_ip and self.policy_dict[policy]['dst_addr'] == dst_ip:
               ...
               if self.policy_dict[policy]['application'] == application and self.policy_dict[policy]['action'] == action:
                  ...
                  src_zone_name = self._get_zone_for_ip(src_ip)
                  dst_zone_name = self._get_zone_for_ip(dst_ip)
                  src_addr,dst_addr = self._get_addr_for_ip(src_ip, dst_ip)
